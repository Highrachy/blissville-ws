...
function upload_image($file_upload,$destination="images/",$resizeto="300",$resizeby="w"){
	
	define('QUALITY', 80);
	
	$new_image_name = rename_file($_FILES[$file_upload]['name'],$destination);
...
function upload_file($file_upload,$destination="./images"){
	
	# - Check if the file exist on the server, rename if necessary
	$new_file_name = rename_file($_FILES[$file_upload]['name'],$destination);
